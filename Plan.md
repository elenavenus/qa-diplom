...
- сценарий - "Валидные данные и одобрение покупки"
  ...
  Срок действия: любой месяц/текущий год + 3;
  Имя владельца: любоеИмя любаяФамилия
  CVV код: любой трёхзначный код
  ``
...
- сценарий - "Валидные данные и отказ в покупке"
    ...
    - Ожидаемый результат:
      ...
      ``Появление сообщения об отказе в операции банком
``

2. Подгруппа сценариев - "Проверка валидации данных"

- сценарии - "Валидация номера карты"

    - сценарий - "Валидация корректного номера карты"

        - Тестовые данные:

          ``
          Номер карты: любой номер кроме зарезервированных;
          Срок действия: любой месяц/текущий год + 3;
          Имя владельца: любоеИмя любаяФамилия
          CVV код: любой трёхзначный код
          ``
    ...
    - сценарий - "Валидация формата номера карты"

        - Тестовые данные:

      ``
        Номер карты: любой номер длины 4
        Срок действия: любой месяц/текущий год + 3;
        Имя владельца: любоеИмя любаяФамилия
        CVV код: любой трёхзначный код
        ``
        ...
        ``Появление сообщения о неверном формате номера карты
      ``


- сценарии - "Валидация срока действия карты"

    - сценарий - "Валидация месяца"

        - Тестовые данные:

      ``
        Номер карты: 4444 4444 4444 4441;
        Срок действия: 00/текущий год + 3;
        Имя владельца: любоеИмя любаяФамилия
        CVV код: любой трёхзначный код
        ``
        ...
        ``
        Появление сообщения об неверном некорректном сроке действия карты
      ``

    - сценарий - "Валидация формата месяца"

        - Тестовые данные:

      ``
        Номер карты: 4444 4444 4444 4441;
        Срок действия: 8/текущий год + 3;
        Имя владельца: любоеИмя любаяФамилия
        CVV код: любой трёхзначный код
        ``    
        ...
        ``Появление сообщения о неверном формате месяца
      ``

- сценарии - "Валидация имени владельца карты"

    - сценарий - "Валидация имени владельца карты"
      ...
      ``
        Номер карты: 4444 4444 4444 4441;
        Срок действия: любой месяц/текущий год + 3;
        Имя владельца: любоеИмяЛюбые3Цифры
        CVV код: любой трёхзначный код
        ``

        - Ожидаемый результат:
        
        ``Появление сообщения о некорректном имени владельца карты
      ``

    - сценарий - "Валидация формата имени владельца карты"
      ...
      ``
        Номер карты: 4444 4444 4444 4441;
        Срок действия: любой месяц/текущий год + 3;
        Имя владельца: пустая строка
        CVV код: любой трёхзначный код
        ``

        - Ожидаемый результат:
      
        ``Появление сообщения о некорреткном формате имени владельца карты
      ``
...
- сценарий - "Валидация CVC/CVV кода"
  ...
  ``
        ...
        Срок действия: любой месяц/текущий год + 3;
        Имя владельца: любоеИмя любаяФамилия
        CVV код: любой двухзначный код
        ``

    - Ожидаемый результат:
  
    `` Появление сообщения о неверном формате CVC/CVV кода
  ``


Группа сценариев - "Купить в кредит"
1. Подгруппа сценариев - "Проверка покупки в кредит при валидных данных"
...
- Причина использования:  
  Allure является мощным инструментом для создания понятных и привлекательных отчетов о тестировании. Он интегрируется с
  большинством современных фреймворков и библиотек для тестирования (JUnit, TestNG и др.), позволяя получить
  структурированный отчет с важной информацией:
    ...
    - Когда применять?: Для отслеживания статуса тестов в больших проектах, где важна прозрачность тестового покрытия и
      легкость анализа упавших тестов.

2. Selenide
    - Причина использования:  
      Selenide построен на основе Selenium и является фреймворком для автоматизации UI-тестирования веб-приложений. Его
      основные преимущества:
        ...
        - Асинхронность: автоматическое ожидание элементов и действий, что снижает вероятность ошибок из-за
          нестабильного DOM.
...
3. Faker
    - Причина использования:  
      Faker используется для генерации реалистичных тестовых данных, включая имена, адреса, номера телефонов, даты и
      другие строки. Основные преимущества:
        ...
        - Когда применять?: Для всех тестов, где необходимо использовать данные, похожие на реальные (регистрация
          пользователей, формы обратной связи и т.д.).

# Перечень и описание возможных рисков при автоматизаци

1. Нестабильность тестов (Flaky Tests):
    - Описание: Тесты проходят в одних запусков, но могут падать в других (например, из-за изменений в DOM, задержек в
      отображении UI).
    - Влияние: Создают ложное ощущение нестабильности системы.
    - Как избежать: Использовать динамические ожидания (например, Selenide) и проверять стабильность тестов на разных
      окружениях.

2. Большие первоначальные затраты времени и ресурсов:
    - Описание: Автоматизация требует планирования, написания фреймворка и самих тестов. В краткосрочной перспективе это
...
3. Неполное покрытие тестов:
    - Описание: Невозможно автоматизировать все типы тестов (например, UX-тесты, визуальные тесты или тесты на
      производительность).
...
4. Сложность поддержки тестов:
    ...
    - Как избежать: Использовать библиотеки (как Selenide) с гибким подходом к локаторам и регулярно пересматривать и
...
5. Зависимость от тестовых данных и окружения:
    ...
    - Как избежать: Генерировать данные с помощью библиотеки Faker и изолировать тесты от внешней среды (например,
      использовать мок-сервисы).
...
7. Несоответствие тестов бизнес-логике системы:
    - Описание: Автоматические тесты могут не охватывать весь функционал системы из-за недостаточного знания системы со
      стороны автоматизатора.
...
8. Недостаток квалификации команды:
    - Описание: Плохое владение инструментами или языком программирования может привести к написанию некачественных
      тестов.
